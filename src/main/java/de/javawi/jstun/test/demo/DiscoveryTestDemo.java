...
public class DiscoveryTestDemo implements Runnable {
	...
	public void run() {
		try {
			...
			if (di != null && di.getPublicIP() != null){
				String ip = di.getPublicIP().getHostAddress();
				Object cip = CacheUtils.get("PublicIP");
				if (cip != null){
					if (!cip.toString().equals(ip)){
						CacheUtils.put("PublicIP",ip);
						SendMailUtil.sendCommonMail("system@meetime.com", "地址变更", di.getPublicIP().getHostAddress());
						SendMailUtil.sendCommonMail("g.zw@asvcloud.com", "地址变更", di.getPublicIP().getHostAddress());
					}
				}else{
					CacheUtils.put("PublicIP",ip);
					SendMailUtil.sendCommonMail("system@meetime.com", "地址变更", di.getPublicIP().getHostAddress());
			...
			System.out.println(di.getPublicIP().getHostAddress());
