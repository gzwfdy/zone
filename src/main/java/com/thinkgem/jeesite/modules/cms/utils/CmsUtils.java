...
public class CmsUtils {
	...
	public static List<Category> getMainNavList(String siteId){
		@SuppressWarnings("unchecked")
		List<Category> mainNavList = (List<Category>)CacheUtils.get(CMS_CACHE, "mainNavList_"+siteId+"_"+ UserUtils.getLocale().toString());
		if (mainNavList == null){
			...
			CacheUtils.put(CMS_CACHE, "mainNavList_"+siteId+"_"+ UserUtils.getLocale().getLanguage(), mainNavList);
		}
		return mainNavList;
	}
    ...
    public static String getUrlDynamic(Article article) {
		...
		Site site = CmsUtils.getSite(article.getCategory().getSite().getId());
        str.append(context.getContextPath()).append(StringUtils.isNotBlank(site.getDomain()) ? site.getDomain() : Global.getFrontPath());
    ...
    public static String getUrlDynamic(Category category) {
        if(StringUtils.isNotBlank(category.getHref())){
            if(!category.getHref().contains("://")){
				if(StringUtils.isNotBlank(category.getSite().getDomain()))
					return context.getContextPath()+category.getSite().getDomain()+category.getHref();
				else
        ...
        StringBuilder str = new StringBuilder();
        str.append(context.getContextPath()).append(StringUtils.isNotBlank(category.getSite().getDomain()) ? category.getSite().getDomain() : Global.getFrontPath());
