...
.controller('ConferenceController',
    function(
            ...
            if (!$scope.platformSettings.hasWebRTC) {
                ...
                $timeout(function() {
                    $scope.call.startCall(
                        ...
                        $localStorage.cameraSourceId,
                        $localStorage.microphoneSourceId);
                }, 500); // Make sure the preview has released media
            }
        };

        $scope.startAudioCall = function() {
            $scope.connection.connectingMedia = true;
            $scope.toggle('popup-escalate', false);
            $scope.sendKeenEvent('escalate', {
                    uuid: $scope.connection.data.uuid,
                    audio: true,
                    video: false
        ...
        if ($scope.params.media || $scope.localStorage.startMedia) {
